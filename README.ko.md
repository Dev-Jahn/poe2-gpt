...
**상태: 0.8.0, 실험적 버전.** 최대 26개의 MCP 도구를 구현했습니다. Homelab 배포·인증·본인 캐릭터 검증은 설치 후 진행해야 합니다. 이 저장소는 운영 중인 공용 서버나 ChatGPT 공개 디렉터리 등록을 제공하지 않습니다.
...
엔진은 고정된 PoB2 개발 소스와 해시를 검증한 0.5.5 데이터 패치를 사용합니다. 선택 스킬과 플레이어·선택 소환수 DPS를 구분하며, FullDPS는 설정된 경우에만 제공합니다. 미확인 패시브·미해석 효과·지원하지 않는 설정은 결과에 표시하고 검증된 업그레이드 추천에서 제외합니다. [엔진 호환 범위](docs/pob-engine.md) · [한국어 명칭 수록 범위](docs/localization.md).

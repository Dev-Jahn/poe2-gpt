...
def test_currency_facet_includes_leading_orb_names():
    mod = updater()
    assert "currency" in mod.categories("Orb_of_Annulment", "Orb of Annulment")
    assert "currency" in mod.categories("Mirror_of_Kalandra", "Mirror of Kalandra")


def test_paired_canonical_title_beats_language_and_weapon_navigation(tmp_path):
    import hashlib
    mod = updater()
    html = {
        "us": '<title>Monk - PoE2DB, Path of Exile Wiki us</title>'
              '<a class="dropdown-item" href="Monk">US English</a>'
              '<a href="Monk">Quarterstaves</a>',
        "kr": '<title>몽크 - PoE2DB, Path of Exile Wiki kr</title>'
              '<a class="dropdown-item" href="Monk">KR 한국어</a>'
              '<a href="Monk">육척봉</a>',
    }
    sources = []
    for index, (language, page) in enumerate(html.items()):
        raw = page.encode()
        filename = f"page-{index:02}-{language}.html"
        (tmp_path / filename).write_bytes(raw)
        sources.append({"language": "en" if language == "us" else "ko", "url": f"https://poe2db.tw/{language}/Monk",
                        "file": filename, "sha256": hashlib.sha256(raw).hexdigest()})
    manifest = {"snapshot_date": "2026-09-08", "requested_pages_not_found": [], "sources": sources}
    (tmp_path / "html-provenance.json").write_text(json.dumps(manifest))
    output = tmp_path / "canonical-result.json"
    assert mod.build_html(tmp_path, output, "0.5.5")["record_count"] == 1
    assert Catalog(output).lookup("Monk").ko == "몽크"
    assert not any(r["label"] == "KR 한국어" for r in mod.anchors(html["kr"].encode(), "https://poe2db.tw/kr/Monk", "kr"))
    # Conflicting canonical titles remain unresolved; never choose the first.
    raw = '<title>다른 이름 - PoE2DB, Path of Exile Wiki kr</title>'.encode()
    (tmp_path / "page-02-kr.html").write_bytes(raw)
    sources.append({"language": "ko", "url": "https://poe2db.tw/kr/Monk", "file": "page-02-kr.html",
                    "sha256": hashlib.sha256(raw).hexdigest()})
    (tmp_path / "html-provenance.json").write_text(json.dumps(manifest))
    with pytest.raises(ValueError, match="empty_bilingual_catalog"):
        mod.build_html(tmp_path, output, "0.5.5")
